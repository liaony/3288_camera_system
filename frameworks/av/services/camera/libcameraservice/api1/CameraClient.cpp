...
status_t CameraClient::sendCommand(int32_t cmd, int32_t arg1, int32_t arg2) {
    ...
    if (cmd == CAMERA_CMD_SET_DISPLAY_ORIENTATION) {
        ...
        //luyq add start 20170508
    char intervalModuleValue[PROPERTY_VALUE_MAX];
    property_get("ro.internal.model", intervalModuleValue, "0");
    if (strcmp(intervalModuleValue, "TPS618") == 0) {
        if(mCameraFacing == CAMERA_FACING_FRONT){
        	orientation =  0; //luyq add
        }else {
        	orientation = getOrientation(arg1, mCameraFacing == CAMERA_FACING_FRONT);
        }
        //luyq add end 20170508
    }else {
    	  orientation = getOrientation(arg1, mCameraFacing == CAMERA_FACING_FRONT);
    }

	//yexb
	//orientation = HAL_TRANSFORM_FLIP_H; //luyq comment
