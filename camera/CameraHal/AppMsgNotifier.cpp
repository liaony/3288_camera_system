...
int AppMsgNotifier::captureEncProcessPicture(FramInfo_s* frame){
	...
	LOGD("rawbuf_phy:%x,rawbuf_vir:%x;jpegbuf_phy = %x,jpegbuf_vir = %x,liaony",rawbuf_phy,rawbuf_vir,jpegbuf_phy,jpegbuf_vir);
  #if 0
	...
	LOGD("captureEncProcessPicture,rotation = %d,jpeg_w = %d,jpeg_h = %d,liaony",rotation,jpeg_w,jpeg_h);
#if 0  // 之前
			{
				FILE* fp =NULL;
				char filename[40];
				static char cnt = 0;
				filename[0] = 0x00;
				sprintf(filename, "/data/camera/bf_%dx%d_%d.yuv",frame->frame_width,frame->frame_height, cnt++);
				fp = fopen(filename, "wb+");
				if (fp > 0) {
					fwrite((char*)frame->vir_addr, 1,frame->frame_width*frame->frame_height*3/2,fp);
					fclose(fp);
 ...
 // 之后 get yuv data liaony
		{
			    FILE* fp =NULL;
			    char filename[40];
			    filename[0] = 0x00;
			    static char af_cnt = 0;
			    sprintf(filename, "/data/camera/af_%dx%d_%d.yuv",jpeg_w,jpeg_h,af_cnt++);
			    fp = fopen(filename, "wb+");
			    if (fp > 0) {
				    fwrite((char*)rawbuf_vir, 1,jpeg_w*jpeg_h*3/2,fp);
				    fclose(fp);
				    LOGD("Write success yuv data to %s,liaony",filename);
			    } else {
				    LOGE("Create %s failed(%d, %s),liaony",filename,fp, strerror(errno));
			    }
    ...
    if((frame->frame_fmt == V4L2_PIX_FMT_NV12) && ((frame->frame_width != mPictureInfo.w) || (frame->frame_height != mPictureInfo.h) || (frame->zoom_value != 100) || (long)frame->phy_addr == -1)){
		...
		LOGD("liaony jpeg_w = %d,jpeg_h = %d",jpeg_w,jpeg_h);
    ...
    output_vir_addr = jpegbuf_vir;
#if 9 // 之后 liaony@code
		    {
			    ...
			    filename[0] = 0x00;
			    static char af_cnt = 0;
			    sprintf(filename, "/data/camera/bf_%dx%d_%d.yuv",jpeg_w,jpeg_h,af_cnt++);
			    fp = fopen(filename, "wb+");
			    if (fp > 0) {
		...
		while (loop) {
			...
			switch (msg.command)
			{
				case EncProcessThread::CMD_ENCPROCESS_SNAPSHOT:
				{
					...
					LOGD("%s(%d):captureEncProcessPicture!liaony",__FUNCTION__,__LINE__);
                    //return frame
