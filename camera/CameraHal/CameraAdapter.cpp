...
int CameraAdapter::initialize()
{
    ...
    LOG_FUNCTION_NAME
    //mv_camera_dev();
    //sprintf(cmd, "mv /dev/video1 /dev/video0");
	//popen(cmd,"r");
	if((ret = cameraCreate(mCamId)) < 0){
	//	control_camera_power(2);
	//	usleep(100000);
	//	control_camera_power(1);
	//	usleep(1500000);
	//	if((ret = cameraCreate(mCamId)) < 0){
	//		sleep(100);
			return ret;
...
//@liaony 20181030
int CameraAdapter::control_camera_power(int powerID)
{
	FILE * fd = NULL ;
	int ret = -1;
	char cmd[50] = "0";
...
//@liaony 20181204
int CameraAdapter::mv_camera_dev(void)
{
	FILE *fd = NULL ;
	int ret = -1;
	char cmd[50] = "0";
	char *video0_path = "/dev/video0";
	char *video1_path = "/dev/video1";
	sprintf(cmd, "mv /dev/video1 /dev/video0");
	//fstream f1,f2;
	//if((!f1.open(video0_path))&&(f2.open(video1_path)){
	if(((access( video0_path, 0 )) == -1) && ((access( video1_path, 0 )) != -1)){
		//if(fd = popen(cmd,"r") == NULL)
		popen(cmd,"r");
		LOGE("Can't not set video");
		}
	pclose(fd);
	return ret;
}
...
int CameraAdapter::cameraCreate(int cameraId)
{
    ...
    if (CameraHal_SupportFmt[i] == 0x00) {
        ...
        LOGD("mCamDriverPreviewFmt  = %d,and %c%c%c%c",mCamDriverPreviewFmt,mCamDriverPreviewFmt & 0xFF, (mCamDriverPreviewFmt >> 8) & 0xFF,
			(mCamDriverPreviewFmt >> 16) & 0xFF, (mCamDriverPreviewFmt >> 24) & 0xFF);
...
int CameraAdapter::getFrame(FramInfo_s** tmpFrame){
   ...
   int ret = 0;
    ...
    if (ioctl(mCamFd, VIDIOC_DQBUF, &cfilledbuffer1) < 0) {
    	...
    	//LOGE("%s(%d):deque a frame %d OK,size:%d, H:%d, W:%d,liaony",__FUNCTION__,__LINE__,cfilledbuffer1.index,cfilledbuffer1.bytesused,mCamDrvHeight,mCamDrvWidth);

    if(mCamDrvHeight==1944|| mCamDrvWidth==2592){
	if(cfilledbuffer1.bytesused < 80000)
		LOGE("500W resolution size of buffer ERROR!liaony");
	}
