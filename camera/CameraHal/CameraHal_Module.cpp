...
//liaony @20181025
static int camera_power_control(int powerID)
{
	FILE * fd = NULL ;
	int ret = -1;
	char cmd[50] = "0";
	...
	if (powerID == 1){
		ret = 1;
		sprintf(cmd, "echo 1 > /sys/class/telpoio/power_status");
		if((fd = popen(cmd,"r")) != NULL)
			LOGE("Control camera power off!");
	}
	else if (powerID == 2){
		ret = 2;
		sprintf(cmd, "echo 2 > /sys/class/telpoio/power_status");
		if((fd = popen(cmd,"r") ) != NULL)
			LOGE("Control camera power on!");
	}
	else if (fd == NULL){
		ret = -1;
		LOGE("Can not control telpoio power_status!");
	}
	pclose(fd);
#endif
	return ret;
    ...
    if (nCamDev>0) {
        ...
        {  
        	...
        	if(profiles->mDevieVector[i]->mIsConnect==1){
              ...
              //luyq modify start      
						  //-----------USBCAMERA_FRONT_STAR_START---------------						  
						  //-----------USBCAMERA_FRONT_STAR_END---------------	
    	        ...
    	        if(pSensorInfo->mFacing == RK_CAM_FACING_FRONT){     
    	            ...
    	            camInfoTmp[cam_cnt].facing_info.facing = CAMERA_FACING_BACK;
						  ...
						  //-----------USBCAMERA_FRONT_TAIL_STAR_START---------------						  
						  //-----------USBCAMERA_FRONT_TAIL_STAR_END---------------

//---------------------------------------------------------------------------------------------------------------------------------------------

						  //-----------USBCAMERA_BACK_STAR_START---------------						  
/*
						  //-----------USBCAMERA_BACK_STAR_END---------------
		         ...
		         if(strcmp((char*)&capability.driver[0],"uvcvideo") == 0)//uvc
		         {
		             camInfoTmp[cam_cnt&0x01].facing_info.facing = CAMERA_FACING_FRONT;
		        	   ...
		        	   camInfoTmp[cam_cnt&0x01].facing_info.facing = CAMERA_FACING_BACK;
						  ...
						  //-----------USBCAMERA_BACK_TAIL_STAR_START---------------						  
*/
						  //-----------USBCAMERA_BACK_TAIL_STAR_END---------------
   	...
   	if(cam_cnt<CAMERAS_SUPPORT_MAX){
        ...
        for (i=0; i<2; i++) {
            cam_path[0] = 0x00;
			unsigned int pix_format_tmp = V4L2_PIX_FMT_MJPEG;
            ...
            if ((capability.capabilities & (V4L2_CAP_VIDEO_CAPTURE | V4L2_CAP_STREAMING)) != (V4L2_CAP_VIDEO_CAPTURE | V4L2_CAP_STREAMING)) {
		        ...
		        //luyq add start 20170706 usb camera front
		       /* if(strcmp((char*)&capability.driver[0],"uvcvideo") == 0)//uvc
		        {
		             camInfoTmp[cam_cnt&0x01].facing_info.facing = CAMERA_FACING_FRONT;
		        }else {
				...
				if(strcmp((char*)&capability.driver[0],"uvcvideo") == 0)//uvc
				{
					...
					while (ioctl(fd, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
						...
						LOGD("mCamDriverSupportFmt:(%c%c%c%c),index = %d",fmtdesc.pixelformat&0xFF,(fmtdesc.pixelformat>>8)&0xFF,
							(fmtdesc.pixelformat>>16)&0xFF,(fmtdesc.pixelformat>>24)&0xFF,fmtdesc.index);
						LOGD("mCamDriverSupportFmt:fmt = %d,index = %d",mCamDriverSupportFmt[fmtdesc.index],fmtdesc.index);
						fmtdesc.index++;
					}
					...
					while (CameraHal_SupportFmt[i]) {
						...
						while (mCamDriverSupportFmt[j]) {
							//LOGD("CameraHal_SupportFmt:fmt = %d,index = %d  mCamDriverSupportFmt = %d ,i);
					...
					if (CameraHal_SupportFmt[i] == 0x00) {
						...
						LOGE("mCamDriverPreviewFmt = %d, no support fmt",mCamDriverPreviewFmt); //liaony
						goto loop_continue;
					} else {  
					...
					{
						LOGD("mCamDriverPreviewFmt = %d, no fmt support so go to end",mCamDriverPreviewFmt);
						goto loop_continue;
					}
					//*******************************
					fsize.index = 0;
...
loop_continue:
    ...
    if((strcmp(camInfoTmp[0].driver,"uvcvideo") == 0)) {
        if (cam_cnt == 1) {
            ...
            camInfoTmp[0].facing_info.facing = (camInfoTmp[1].facing_info.facing == CAMERA_FACING_FRONT) ? CAMERA_FACING_BACK:CAMERA_FACING_FRONT;//luyq modify
        }
...
int camera_get_camera_info(int camera_id, struct camera_info *info)
{
    ...
    LOGE("%s camera_id = %d, num supported = %d,liaony",__FUNCTION__,camera_id, gCamerasNumber);    
