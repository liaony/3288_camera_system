...
//			}
            ...
            while ((ret = ioctl(mCamFd, VIDIOC_ENUM_FRAMEINTERVALS, &fival)) == 0) {
                if (fival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
                    ...
                    LOGD("%dx%d : %d  %d/%d,liaony",fival.width,fival.height, framerate[i-1],fival.discrete.denominator,fival.discrete.numerator);
    ...
    params.set(KEY_PREVIEW_H_FORCE,"0");
        //luyq add 
  	LOGD("%s(%d):USBCamera parameterString.string()=%s",__FUNCTION__,__LINE__,parameterString.string());

    //1280x960,2592x1944,800x600,352x288,320x240,176x144,160x120,1024x768,1280x720,1920x1080,1600x1200,640x480
    //parameterString="1280x960,2592x1944,800x600,352x288,320x240,176x144,160x120,1024x768,1280x720,1920x1080,1600x1200,640x480";//luyq add
    ...
    //luyq add 
  	LOGD("%s(%d):USBCamera parameterString.string()2=%s",__FUNCTION__,__LINE__,parameterString.string());
   ...
   /*supported size of the EXIF thumbnail in Jpeg picture setting */
    ...
    LOGD ("USBCamera Support Preview format: %s .. %s",params.get(CameraParameters::KEY_SUPPORTED_PREVIEW_FORMATS),
        params.get(CameraParameters::KEY_PREVIEW_FORMAT));
    LOGD ("USBCamera Support Preview sizes: %s     %s",params.get(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES),
        params.get(CameraParameters::KEY_PREVIEW_SIZE));
    LOGD ("USBCamera Support Preview FPS range: %s",params.get(CameraParameters::KEY_SUPPORTED_PREVIEW_FPS_RANGE));
...
int CameraUSBAdapter::setParameters(const CameraParameters &params_set,bool &isRestartValue)
{
    ...
    if (strstr(mParameters.get(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES), params.get(CameraParameters::KEY_PREVIEW_SIZE)) == NULL) {
        ...
        LOGD("%s(%d): Set preview size %s,liaony",__FUNCTION__,__LINE__,params.get(CameraParameters::KEY_PREVIEW_SIZE));
        if(mPreviewRunning){
    ...
    if (strstr(mParameters.get(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES), params.get(CameraParameters::KEY_PICTURE_SIZE)) == NULL) {
        ...
        LOGD("%s(%d): Set picture size %s,liaony",__FUNCTION__,__LINE__,params.get(CameraParameters::KEY_PICTURE_SIZE));
    }
	...
	if (!cameraConfig(params,false,isRestartValue)) {        
        ...
        LOGD("Scene: %s", params.get(CameraParameters::KEY_SCENE_MODE));
...
#if 0 // 之后 liaony@code
		if ((frame->frame_width == 2592) && (frame->frame_height == 1944))
		    {
			    FILE* fp =NULL;
			    char filename[40];
			    filename[0] = 0x00;
			    static char jpg_cnt = 0;
			    sprintf(filename, "/data/camera/%dx%d_%d.jpg",frame->frame_width,frame->frame_height,jpg_cnt++);
			    fp = fopen(filename, "wb+");
			    if (fp > 0) {
				    fwrite((char*)frame->vir_addr, 1,input_len,fp);
				    fclose(fp);
				    LOGD("%s(%d): Write success yuv data to %s,input_len is %d ,liaony",__FUNCTION__,__LINE__,filename,input_len);
			    } else {
		...
		if ((w == 2592) && (h == 1944))
		    {
			    ...
			    char filename[40];
			    filename[0] = 0x00;
			    static char yuv_cnt = 0;
			    sprintf(filename, "/data/camera/af_dec_%dx%d_%d.yuv",w,h,yuv_cnt++);
			    fp = fopen(filename, "wb+");
			    if (fp > 0) {
				    fwrite((char*)frame->vir_addr, 1,w*h*3/2,fp);
				    fclose(fp);
				    LOGD("%s(%d): Write success yuv data to %s,liaony",__FUNCTION__,__LINE__,filename);
			    } else {
				    LOGE("Create %s failed(%d, %s)",filename,fp, strerror(errno));
			    }
