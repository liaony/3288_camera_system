...
static int vpu_dmabuf_alloc(struct vpu_dmabuf_dev *idev, size_t size, VPUMemLinear_t **data)
{
    ...
    //1: memory manage functions is non-reentrant
    //2: io-operation functions is non-reentrant;
    //   vpu_mem_judge_used_heaps_type contains io-operations
#if ENABLE_MEMORY_MANAGE
    pthread_mutex_lock(&dev->list_lock);
#endif
    mem_heap_type = vpu_mem_judge_used_heaps_type();
    ...
    err = ion_alloc(dev->ion_client, size, dev->align, mem_heap_type, 0, &dmabuf->handle);
